...
def _privileged_outbox_view_sql(role_expression: str) -> str:
    """Build a cycle-safe probe for reachable views that can bypass outbox RLS."""
    return (
        "EXISTS ("
        "WITH RECURSIVE reachable_view(view_oid, caller_oid) AS ("
        "SELECT exposed_view.oid, "
        f"{role_expression} "
        "FROM pg_catalog.pg_class AS exposed_view "
        ...
        "UNION "
        "SELECT nested_view.oid, reachable_view.caller_oid "
        "FROM reachable_view "
        "JOIN pg_catalog.pg_class AS current_view "
        "ON current_view.oid OPERATOR(pg_catalog.=) reachable_view.view_oid "
        ...
        "ON nested_view.oid OPERATOR(pg_catalog.=) current_view_dependency.refobjid "
        "JOIN pg_catalog.pg_namespace AS nested_view_schema "
        "ON nested_view_schema.oid OPERATOR(pg_catalog.=) nested_view.relnamespace "
        "WHERE nested_view.relkind::pg_catalog.text OPERATOR(pg_catalog.=) 'v' "
        "AND nested_view_schema.nspname NOT LIKE 'pg\\_%' ESCAPE '\\' "
        "AND nested_view_schema.nspname OPERATOR(pg_catalog.<>) 'information_schema' "
        "AND pg_catalog.has_table_privilege("
        "CASE WHEN COALESCE("
        "current_view.reloptions OPERATOR(pg_catalog.@>) "
        "ARRAY['security_invoker=true']::pg_catalog.text[], false) "
        "THEN reachable_view.caller_oid ELSE current_view.relowner END, "
        "nested_view.oid, 'SELECT')"
        ") "
        "SELECT 1 FROM reachable_view "
        "JOIN pg_catalog.pg_class AS exposed_view "
        "ON exposed_view.oid OPERATOR(pg_catalog.=) reachable_view.view_oid "
...
def _require_rls_application_role(cursor: Any) -> None:
    ...
    cursor.execute(
        ...
        "AND (definer_admin_role.rolsuper "
        "OR definer_admin_role.rolcreatedb "
        "OR definer_admin_role.rolcreaterole "
        "OR definer_admin_role.rolreplication "
        "OR definer_admin_role.rolbypassrls "
        "OR definer_admin_role.oid OPERATOR(pg_catalog.=) admitted_relation.relowner "
        "OR pg_catalog.pg_has_role("
        "definer_admin_role.oid, admitted_relation.relowner, 'USAGE') "
        "OR pg_catalog.pg_has_role("
        "definer_admin_role.oid, admitted_relation.relowner, 'SET') "
        "OR pg_catalog.pg_has_role("
        "definer_admin_role.oid, admitted_relation.relowner, "
        "'MEMBER WITH ADMIN OPTION') "
        "OR pg_catalog.has_any_column_privilege("
        ...
        "OR pg_catalog.has_table_privilege("
        "definer_admin_role.oid, admitted_relation.oid, 'DELETE') "
        "OR pg_catalog.has_any_column_privilege("
        "definer_admin_role.oid, admitted_relation.oid, 'UPDATE') "
        "OR pg_catalog.has_any_column_privilege("
        "definer_admin_role.oid, admitted_relation.oid, 'REFERENCES') "
        "OR pg_catalog.has_table_privilege("
        "definer_admin_role.oid, admitted_relation.oid, 'TRIGGER') "
        "OR EXISTS ("
        "SELECT 1 FROM pg_catalog.pg_roles AS definer_admin_set_role "
        "WHERE pg_catalog.pg_has_role("
        "definer_admin_role.oid, definer_admin_set_role.oid, 'SET') "
        "AND (definer_admin_set_role.rolsuper "
        "OR definer_admin_set_role.rolcreatedb "
        "OR definer_admin_set_role.rolcreaterole "
        "OR definer_admin_set_role.rolreplication "
        "OR definer_admin_set_role.rolbypassrls "
        "OR definer_admin_set_role.oid OPERATOR(pg_catalog.=) admitted_relation.relowner "
        "OR pg_catalog.pg_has_role("
        "definer_admin_set_role.oid, admitted_relation.relowner, 'USAGE') "
        "OR pg_catalog.has_any_column_privilege("
        ...
        "OR pg_catalog.has_table_privilege("
        "definer_admin_set_role.oid, admitted_relation.oid, 'DELETE') "
        "OR pg_catalog.has_any_column_privilege("
        "definer_admin_set_role.oid, admitted_relation.oid, 'UPDATE') "
        "OR pg_catalog.has_any_column_privilege("
        "definer_admin_set_role.oid, admitted_relation.oid, 'REFERENCES') "
        "OR pg_catalog.has_table_privilege("
