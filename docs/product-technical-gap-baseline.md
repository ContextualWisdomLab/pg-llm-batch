...
The explicit `pg_service.conf` capability is caller-selected and bounded. It does not perform ambient `PGSERVICEFILE`, user/system service-file, LDAP, or filesystem search discovery. Eight independent authority/compatibility defects have been repaired.
