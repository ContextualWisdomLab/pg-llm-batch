...
requires `uv_build==0.12.12`. The exact uv frontend can use its compatible bundled
...
3. Confirm the workflow uses `uv` 0.12.3 and `uv_build==0.12.12` from the exact
   reviewed source.
...
backend requirement remains exactly `uv_build==0.12.12`. Do not treat
