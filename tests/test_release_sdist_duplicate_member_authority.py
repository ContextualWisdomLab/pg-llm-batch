...
"""Regression contract for unique source-distribution member names."""
...
def _sdist_with_duplicate_member() -> bytes:
    """Return a bounded sdist with two conflicting entries for one package path."""
    output = io.BytesIO()
...
def test_release_verifier_rejects_duplicate_sdist_member_names(tmp_path: Path) -> None:
    """Reject an sdist whose archive inventory assigns one path twice."""
    sdist_bytes = _sdist_with_duplicate_member()
