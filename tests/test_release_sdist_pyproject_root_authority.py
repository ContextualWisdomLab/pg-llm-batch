...
"""Regression contract for the required root pyproject.toml in sdists."""
...
PYPROJECT_CASES = (
    ...
    pytest.param(f"{TOP_LEVEL}/nested/pyproject.toml", id="misplaced"),
)
...
def _sdist_without_root_pyproject(pyproject_member: str | None) -> bytes:
    """Return a bounded sdist missing pyproject.toml from its canonical root."""
    output = io.BytesIO()
...
@pytest.mark.parametrize("pyproject_member", PYPROJECT_CASES)
def test_release_verifier_requires_pyproject_at_sdist_root(
    tmp_path: Path,
    pyproject_member: str | None,
) -> None:
    """Reject standard sdists without pyproject.toml at the package root."""
    sdist_bytes = _sdist_without_root_pyproject(pyproject_member)
