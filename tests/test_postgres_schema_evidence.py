...
class _CloseFailure(BytesIO):
    """Expose one explicit lower-layer close failure without destructor leakage."""
...
class _StateCloseFailure(BytesIO):
    """Model one explicit non-OSError close failure without destructor leakage."""
...
class _ReadAndCloseFailure(_ReadFailure):
    """Fail one read and explicit cleanup while allowing destructor cleanup."""
...
class _ReadAndStateCloseFailure(_ReadFailure):
    """Fail read and one non-OSError cleanup while allowing destructor cleanup."""
