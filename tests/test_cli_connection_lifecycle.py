...
"""Regression tests for CLI-owned PostgreSQL collaborator lifecycles."""
...
class _SecretStore:
    ...
    def __init__(self, dsn: str, fernet_key: str | None = None) -> None:
        """Record the explicit DSN and key presence without retaining a key."""
        self.events.append(("secret-open", dsn, fernet_key is not None))

    def set_secret(self, key: str, value: str) -> None:
        """Record only the secret identifier and value length."""
...
def _install_sync_stores(monkeypatch: Any) -> None:
...
def test_config_commands_close_owned_database_stores(monkeypatch: Any) -> None:
    ...
    assert _SecretStore.events == [
        ...
        ("secret-set", "gateway_api_key.default", 6),
        ("secret-close",),
    ]
...
class _AsyncClient:
    ...
    def __init__(self, dsn: str, provider: Any) -> None:
        ...
        self.events.append(("client-open", dsn, provider))

    async def __aenter__(self) -> "_AsyncClient":
        """Record entry and return this client."""
        self.events.append(("client-enter",))
        return self

    async def __aexit__(self, *_exc: Any) -> None:
        """Record HTTP-client cleanup before store cleanup."""
...
def test_async_cli_client_closes_credential_stores(monkeypatch: Any) -> None:
...
def test_async_client_construction_failure_closes_partial_stores(
    monkeypatch: Any,
) -> None:
