...
The explicit `pg_service.conf` capability is caller-selected and bounded. It does not perform ambient `PGSERVICEFILE`, user/system service-file, LDAP, or filesystem search discovery. Nine independent authority/compatibility defects have been repaired.
...
Sixth, the service parser normalized target keys with `key.strip()` after splitting on `=`, while PostgreSQL libpq trims only line framing and retains bytes before the delimiter as part of the key. This made `host =db.example` an accepted `host` parameter in the candidate even though libpq rejects the whitespace-bearing key. Test-first `97ffca5d93ea6e7e16e94e77dfbbb7eaa0b15975` produced a real Python 3.13 CI RED in job `102356095617`: `test_service_file_rejects_whitespace_before_equals_like_libpq` did not raise, with `1 failed, 1668 passed, 5 deselected`. Minimal repair `e59d7b14fb74671949c781c997010055ff7f848c` rejects a target key when its bytes differ from its stripped form, without trimming the value bytes after `=` or broadening ambient service-file authority.

Seventh, the candidate stripped whitespace *inside* section brackets after line framing, so `[ analytics ]` was silently normalized to requested service `analytics`. PostgreSQL `parseServiceFile` trims only leading/trailing line whitespace and then compares the bytes immediately after `[` with the requested service, requiring `]` immediately after the service bytes. Test-first `cb6ac7baee8df108fe7eda7ffec5272c4ee92fc0` produced a real exact-source Python 3.12 RED in CI `34322197523`, job `102371147113`: `test_service_file_does_not_normalize_whitespace_inside_section_identity` did not raise, with `1 failed, 1669 passed, 5 deselected`. Minimal production repair `160aab427352e3ed4eddfb2298f1ca4b2665158b` removes only the inner section-name `.strip()`, preserving libpq's outer line-framing behavior and the existing bounded fail-closed subset.

Eighth, Python's Unicode-aware `str.strip()` and `splitlines()` were broader than libpq's byte-oriented service-file line framing. In particular, a UTF-8 non-breaking space before `[analytics]` was normalized away and could promote a stanza that libpq does not recognize as that service section. Test-first `c78c91b6df809341c63cbfcaed4136f8b90991f9` produced real exact-source RED in CI `34327267450`: Python 3.12 job `102387298371` failed because `test_service_file_does_not_treat_unicode_space_as_libpq_line_framing` did not raise; the job ended `1 failed, 1670 passed, 5 deselected, 1 warning`. Minimal production repair `50243a352421797500890d95531642fb921ec234` splits only on LF and trims only the explicit ASCII service-line whitespace bytes space/tab/CR/VT/FF before interpreting section or key/value framing. Non-ASCII Unicode whitespace is no longer silently promoted into libpq framing authority.

Ninth, the candidate's 64 KiB whole-file bound did not reproduce libpq's stricter per-line `fgets` bound. PostgreSQL `parseServiceFile` reads into a 1024-byte buffer and rejects any returned line whose `strlen` is at least 1023 bytes. The candidate therefore admitted newline-terminated or final unterminated service lines that libpq rejects, allowing connection parameters to come from a file outside the claimed compatible subset. Test-first lineage `926e4a3fce070c6e42e94ae8c522f0137be1d304` → `597fe116a4b096e483887e89fc841d96319396cc` covers both boundary shapes at byte length 1023. Their hosted jobs were cancelled by ordinary repair publication before a terminal failure and are not counted as hosted RED. The causal mismatch is established directly against PostgreSQL's `parseServiceFile` source and the then-current candidate parser, which had no per-line bound. Minimal production repair `bc36b9941025bf43150680dec2fa8b2a62cc2876` validates raw bytes before UTF-8 decoding, applying the same effective 1023-byte `fgets` rejection threshold to LF-terminated and final unterminated lines while retaining the existing 64 KiB whole-file capability bound.

These filesystem/content-capability and parser-compatibility controls prove bounded selector authority, not remote TLS/CA/hostname policy. Issue #123 remains separate.
...
| Explicit service-file authority | Repaired on #323 / exact hosted revalidation required | Preserve construction-time absolute path, parent identity, selected regular-file identity, selected-content digest, descriptor-relative I/O, libpq-compatible byte-level ASCII line/section/key/value framing, libpq per-line byte bound, bounded parsing, and generic diagnostics through final integration/release. |
...
- explicit service-file capability cannot be redirected by final symlink/path substitution, later CWD changes, replacement of the selected parent directory, replacement of the construction-selected regular-file inode, or in-place mutation of the construction-selected file content, and target line/section/key/value framing plus per-line byte length stay within the verified libpq-compatible subset without Unicode whitespace normalization;
...
PostgreSQL Global Development Group. (2026). *fe-connect.c: `parseServiceFile`* [Source code]. https://github.com/postgres/postgres/blob/master/src/interfaces/libpq/fe-connect.c
