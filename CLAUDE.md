...
- Keep row-level security enabled and forced. Runtime admission must reject
  ...
  rejected when their owner can reintroduce or redistribute forbidden authority
  through superuser, `CREATEDB`, `CREATEROLE`, `REPLICATION`, RLS-bypass status,
  exact or inherited table ownership, grant options, `TRUNCATE`, `DELETE`,
  `UPDATE`, `REFERENCES`, `TRIGGER`, or membership `ADMIN OPTION` over a role
  that directly or through an all-`SET TRUE` path carries the same forbidden
  authority. PostgreSQL lets the membership administrator grant a role onward
  ...
  and reviewed catalog dependencies before tenant binding or outbox SQL.
  `CREATEDB` and `CREATEROLE` are database/role administration capabilities,
  `REPLICATION` is separate cluster-level connection and slot authority whether
  held directly or by a callable definer owner, and direct DML grant options,
  authority-bearing role administration, and executable privileged definer
  authority are outside application DML. Runtime identities remain
