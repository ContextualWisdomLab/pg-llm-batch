...
A third authority review proved that construction-time absolute binding alone still left the selected parent pathname mutable. At exact test-first head `a7bb3ea76ad9747b83f517d518b2ddf22fdd7e92`, the resolver was constructed against `selected/pg_service.conf`; the selected parent was then renamed and a new `selected/` directory containing a different service file was created at the same pathname. CI `34278226597` reached a real unit-test RED: the resolver did not raise and followed the replacement parent. Release Acceptance `34278226626` succeeded separately and therefore did not convert that functional RED into GREEN.

Minimal production repair `3dc5dad088ff8732d9e3b9a715f2f6e7e467e862` retains the selected parent directory's construction-time `(st_dev, st_ino)` identity, opens and authenticates that parent, and performs final-component `lstat()` and `open()` relative to the authenticated directory descriptor. This preserves final-component symlink/inode checks while preventing a pathname-level parent replacement from silently changing database connection authority. The first descendant test adaptation `cb0ab5005540b30ea6dccab759b6ad80c5867d4f` then exposed stale assumptions in two private test fixtures plus uncovered new parent-admission fail-closed branches. Those were repaired without weakening production checks by `62bcf95d2fdd96a03b3b9b22132c319ceec363b2`, `deb85f26e18043c127742d337b02051f3fc49cfe`, and focused coverage descendant `b9dcafb8667b71250b10435fc693fbd678f7322f`.

Exact `b9dcafb...` reached CI `34279367790` and Release Acceptance `34279367778` terminal success. The CI matrix passed Python 3.10/3.11/3.12/3.13/3.14, package/lint/docstrings, component/PostgreSQL images, exact pg8000 dependency/source-digest and permissive-license checks, release-environment installation, and real PostgreSQL pg8000 smokes on Python 3.10/3.12/3.14. The quality lane reported `1657 passed, 5 deselected`, public docstrings exactly 100%, and owned production coverage exactly 100% across 4,677 statements and 1,326 branches with zero misses or partial branches. This evidence belongs to that exact Draft head only; the documentation descendant created by this baseline update must acquire its own exact-head CI and Release Acceptance before #323 can again be called current-head GREEN.

PostgreSQL 18 documents service names as indirection to connection parameters and allows user/system service files plus `PGSERVICEFILE`; this repository intentionally keeps those ambient discovery paths outside the pg8000 adapter and accepts only one caller-selected file. Python 3.14 documents directory-relative path operations through `dir_fd`; the repair uses that mechanism as a capability boundary after authenticating the selected parent identity. Neither decision establishes remote TLS/CA/hostname policy, which remains issue #123 authority.
...
| Explicit service-file authority | Active / repaired on #323 | Preserve final-component inode retention, construction-time CWD binding, retained parent-directory identity, descriptor-relative final-component I/O, bounded parsing, and fixed diagnostics through final exact-head CI, package-installed/runtime acceptance, protected integration, and release. |
...
- explicit service-file capability selection cannot be redirected by a final-component symlink, a different inode substituted before open, a later working-directory change after a relative path has been selected, or replacement of the selected parent directory at the same pathname;
