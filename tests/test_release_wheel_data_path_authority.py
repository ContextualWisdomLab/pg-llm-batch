...
"""Regression contract for canonical wheel ``.data`` path authority."""
...
def _wheel_with_data_root(data_root: str) -> bytes:
    """Return a bounded wheel whose install data uses the supplied root path."""
    metadata_name = f"{DIST_INFO}/METADATA"
    wheel_name = f"{DIST_INFO}/WHEEL"
    record_name = f"{DIST_INFO}/RECORD"
    data_member = f"{data_root}/data/pg_llm_batch-release-note.txt"
    ...
    data_payload = b"release evidence fixture\n"
    payloads = (
        (PACKAGE_MEMBER, package_payload),
        (data_member, data_payload),
...
@pytest.mark.parametrize(
    "data_root",
    (
        "other_project-0.1.0.data",
        "pg_llm_batch-9.9.9.data",
    ),
    ids=("wrong-distribution-directory", "wrong-version-directory"),
)
def test_release_verifier_requires_canonical_wheel_data_path(
    tmp_path: Path,
    data_root: str,
) -> None:
    """Reject wheel install data stored under a non-authoritative data path."""
    sdist_bytes = _matching_sdist()
    wheel_bytes = _wheel_with_data_root(data_root)
