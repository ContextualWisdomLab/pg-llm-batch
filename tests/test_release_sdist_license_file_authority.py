...
"""Regression contract for source-distribution License-File payload authority."""
...
def _sdist_with_invalid_license_location(license_member: str | None) -> bytes:
    """Return a canonical sdist except for its declared license-file payload."""
    output = io.BytesIO()
    with tarfile.open(fileobj=output, mode="w:gz", format=tarfile.PAX_FORMAT) as archive:
        ...
        if license_member is not None:
            _add_regular_member(archive, license_member, LICENSE_PAYLOAD)
...
def _matching_wheel() -> bytes:
    """Return a bounded wheel carrying the declared license at its required path."""
    metadata_name = f"{DIST_INFO}/METADATA"
    wheel_name = f"{DIST_INFO}/WHEEL"
    record_name = f"{DIST_INFO}/RECORD"
    license_name = f"{DIST_INFO}/licenses/{LICENSE_PATH}"
    wheel_metadata = (
        b"Wheel-Version: 1.0\n"
...
@pytest.mark.parametrize(
    ...
    (
        None,
        f"{TOP_LEVEL}/licenses/{LICENSE_PATH}",
    ),
    ids=("missing-declared-license", "license-at-wrong-relative-path"),
)
def test_release_verifier_binds_sdist_license_file_to_metadata_authority(
    tmp_path: Path,
    license_member: str | None,
) -> None:
    """Reject an sdist license file absent from the path declared by PKG-INFO."""
    sdist_bytes = _sdist_with_invalid_license_location(license_member)
