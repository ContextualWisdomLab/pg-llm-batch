...
"""Regression contract for the required root PKG-INFO in source distributions."""
...
PKG_INFO_CASES = (
    ...
    pytest.param(f"{TOP_LEVEL}/metadata/PKG-INFO", id="misplaced"),
)
...
def _sdist_without_root_pkg_info(pkg_info_member: str | None) -> bytes:
    """Return a bounded sdist missing PKG-INFO from its canonical root."""
    output = io.BytesIO()
    with tarfile.open(fileobj=output, mode="w:gz", format=tarfile.PAX_FORMAT) as archive:
        ...
        if pkg_info_member is not None:
            _add_regular_member(archive, pkg_info_member, CORE_METADATA)
        _add_regular_member(
            archive,
...
@pytest.mark.parametrize("pkg_info_member", PKG_INFO_CASES)
def test_release_verifier_requires_pkg_info_at_sdist_root(
    tmp_path: Path,
    pkg_info_member: str | None,
) -> None:
    """Reject standard sdists without PKG-INFO at the package root."""
    sdist_bytes = _sdist_without_root_pkg_info(pkg_info_member)
