...
"""Regression contract for the single top-level directory in source distributions."""
...
EXTRA_TOP_LEVEL_CASES = (
    pytest.param("other_project-0.1.0/README.md", id="second-directory"),
    pytest.param("README.md", id="root-file"),
)
...
def _sdist_with_extra_top_level(extra_member: str) -> bytes:
    """Return a bounded sdist with content outside its canonical package root."""
    output = io.BytesIO()
    with tarfile.open(fileobj=output, mode="w:gz", format=tarfile.PAX_FORMAT) as archive:
        ...
        _add_regular_member(
            archive,
            f"{TOP_LEVEL}/pg_llm_batch/__init__.py",
            b"__version__ = '0.1.0'\n",
        )
        _add_regular_member(archive, extra_member, b"unexpected top-level content\n")
...
@pytest.mark.parametrize("extra_member", EXTRA_TOP_LEVEL_CASES)
def test_release_verifier_requires_single_sdist_top_level_directory(
    tmp_path: Path,
    extra_member: str,
) -> None:
    """Reject sdists containing content outside the single package root."""
    sdist_bytes = _sdist_with_extra_top_level(extra_member)
