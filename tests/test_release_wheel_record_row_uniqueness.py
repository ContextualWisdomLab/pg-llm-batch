...
"""Regression contract for unique wheel RECORD path authority."""
...
def _wheel_with_duplicate_record_path() -> bytes:
    """Return a bounded wheel whose RECORD repeats one otherwise valid member row."""
    ...
    package_row = (
        f"{PACKAGE_MEMBER},sha256={_record_digest(package_payload)},"
...
def test_release_verifier_rejects_duplicate_record_paths(tmp_path: Path) -> None:
    """Require exactly one RECORD row for each represented wheel member path."""
    sdist_bytes = _matching_sdist()
    wheel_bytes = _wheel_with_duplicate_record_path()
