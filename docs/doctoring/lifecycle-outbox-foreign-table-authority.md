...
A fourth mechanism is definer-mediated access. PostgreSQL documents that `SECURITY DEFINER` functions execute with the privileges of their owner. `postgres_fdw` establishes remote connections for the applicable local user mapping. An ordinary runtime caller can therefore have no direct `SELECT` on a foreign table yet execute a safe-search-path user-schema definer whose ordinary owner has `SELECT` on that table and maps to a remote identity with broader row authority. Canonical `search_path = pg_catalog, pg_temp` closes name-substitution authority; it does not remove the owner's legitimate foreign-data capability. Test-first commits `8b6cbc130faee66f22d975f2fcc19330d97d4eab`, `076a7d788e385bcefc9f3de2882293b44bd8eab1`, and `01a93f666cd25c9cb25ed152d17b344ae49b3025` preserve that concrete boundary before the production repair.

## Decision

Runtime admission treats foreign data (`pg_class.relkind = 'f'`) as an opaque authority boundary when it is reachable by the effective/authenticated runtime role directly, through ordinary-view execution, through an ordinary or partitioned inheritance parent, through a callable `SECURITY DEFINER` owner's relation authority, or as definition provenance of a reachable materialized relation. Admission fails before tenant binding and before lifecycle row SQL.
...
Callable user-schema `SECURITY DEFINER` routines already form a cycle-safe owner closure. The causal repair reuses the same reachable relation/materialized/foreign-data probe with `definer_role.oid` as the principal. If a callable definer owner can select any opaque foreign relation or a foreign-bearing parent, or can reach a materialized/ordinary-view path already forbidden by the probe, the runtime credential is rejected even when the runtime caller itself has no direct relation privilege. The implementation does not parse a current function body and does not assume an owner capability is harmless merely because one current routine appears not to invoke it.
...
The implementation deliberately does not parse `postgres_fdw` connection options, user-mapping options, remote role names, remote table names, remote RLS state, partition constraints, or callable routine body text. Those are remote or mutable authority and may not even describe a PostgreSQL server for another FDW. A local allowlist or routine-body allowlist would therefore manufacture assurance the package cannot prove. Workloads that require foreign-data access must separate that authority from the lifecycle-outbox runtime role/connection rather than combine it with this forced-RLS credential or a callable definer owner reachable from it.

This is conservative by design: directly reachable foreign data, foreign-bearing inheritance parents, callable definer owners with such authority, and materialized copies sourced from either are rejected even when they appear unrelated to the outbox, because the package cannot prove that the remote target, routine behavior, or copied relation remains unrelated over the lifetime of the credential. That operational restriction is preferable to silently treating mutable remote identity, parent-only ACLs, caller ACL absence, or copied remote rows as local RLS evidence.
...
- `tests/smoke_context_lifecycle_outbox_security_definer_foreign_authority.sh` creates a runtime caller with no direct foreign-table `SELECT`, an ordinary `NOBYPASSRLS` definer owner with its own foreign-table access, and an owner mapping to a remote `BYPASSRLS` identity. The routine pins `search_path = pg_catalog, pg_temp`, returns both seeded tenants through the owner's foreign authority, and requires runtime admission to fail closed.
- `tests/test_context_lifecycle_outbox_foreign_table_authority.py` pins direct/view foreign discovery, the recursive `pg_inherits` ancestor closure, ordinary/partitioned parent reachability, inherited foreign-source membership for materialized provenance, and the definer-owner relation probe.
- Causal inheritance repair `0c2bf9bb991d3556a2d27de8ae8614b138b06c4b` adds the foreign-ancestor closure without changing tenant data SQL, RLS policy, FDW mappings, or caller ACLs.
- Causal definer repair `7367d4864488b21abfd74b389dba38f208e6c6ca` evaluates that existing relation-authority probe under `definer_role.oid`; it does not change routine bodies, mappings, RLS, tenant data SQL, or caller ACLs.
...
The live admission query now computes a recursive foreign-inheritance ancestor set in addition to the existing relation/view/materialized provenance graph, and it evaluates the same relation-authority envelope under callable definer owners. Issue #307 remains the performance owner: buyer acceptance must include catalog size, inheritance depth and fanout, foreign-leaf count, callable-definer owner cardinality/depth, materialized-source depth, connection acquisition, admission, tenant binding, I/O, and cleanup in p50/p95/p99 measurements. Security catalog work may not be omitted or measured only after an unrealistic warm-up to meet the p95 target.
