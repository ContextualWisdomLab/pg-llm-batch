...
3. a reachable foreign table causes fail-closed runtime admission before tenant binding or lifecycle outbox data SQL.
...
This is intentionally conservative. A reachable foreign table that is currently unrelated to the lifecycle outbox is still rejected because the package cannot prove that its remote target and authorization remain unrelated for the lifetime of the credential. The isolation claim is narrower and auditable: the outbox runtime credential has no selectable foreign-data authority.
...
Rejected. Runtime package code does not own database authorization or integration policy. The package reports an unsafe admission boundary; operators repair roles, ACLs, views, servers, and mappings.

## Verification

- Reality RED: exact head `6ccc5da4840a9501ca11d9d38150c7a04ec3408c`, CI run `34077132444`, PostgreSQL/container job `101605370280`.
- Static contract: `tests/test_context_lifecycle_outbox_foreign_table_authority.py` requires direct and ordinary-view traversal of `relkind = 'f'`.
- Executable acceptance: `tests/smoke_context_lifecycle_outbox_foreign_table_authority.sh` proves the cross-tenant foreign result before requiring package admission to reject direct and view-hidden paths.
- Existing role, RLS-policy, `SECURITY DEFINER`, ordinary-view, materialized-view, replay, migration, and package gates remain mandatory.
