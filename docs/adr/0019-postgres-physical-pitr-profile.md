...
- `wal_archive_required=False` on a `physical` profile means no continuous WAL
  archive. Crash-consistent restore still requires backup-internal WAL from
  `pg_basebackup -X stream` or `-X fetch`.
- `pitr` plus `immediate` is `recovery_target=immediate`, not replay to the end
  of the archive.
...
- Profile parse wraps UTF-8 encoding failures, including lone surrogates, as
  `PostgresPhysicalRecoveryError` so hosts catching only that type see a
