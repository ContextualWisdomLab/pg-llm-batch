...
"""Regression contract for invalid special members in source distributions."""
...
def _sdist_with_special_member(member_type: bytes) -> bytes:
    """Return a bounded sdist containing one device or pipe archive entry."""
    output = io.BytesIO()
    with tarfile.open(fileobj=output, mode="w:gz", format=tarfile.PAX_FORMAT) as archive:
        ...
        member = tarfile.TarInfo(f"{TOP_LEVEL}/special-member")
...
@pytest.mark.parametrize("member_type", SPECIAL_MEMBER_TYPES)
def test_release_verifier_rejects_sdist_device_and_pipe_members(
    tmp_path: Path,
    member_type: bytes,
) -> None:
    """Reject reproducible sdists containing device files or pipes."""
    sdist_bytes = _sdist_with_special_member(member_type)
