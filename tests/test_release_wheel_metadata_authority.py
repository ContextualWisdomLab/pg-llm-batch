...
"""Regression contract for wheel-internal compatibility and build authority."""
...
def _wheel_with_metadata(wheel_metadata: bytes) -> bytes:
    """Return a bounded wheel with valid identity metadata and supplied WHEEL metadata."""
...
@pytest.mark.parametrize(
    ...
    ids=("compatibility-tag-mismatch", "spurious-build-tag"),
)
def test_release_verifier_binds_wheel_metadata_to_filename_authority(
    tmp_path: Path,
    wheel_metadata: bytes,
) -> None:
    """Reject WHEEL compatibility/build metadata that contradicts the filename."""
    sdist_bytes = _matching_sdist()
    wheel_bytes = _wheel_with_metadata(wheel_metadata)
