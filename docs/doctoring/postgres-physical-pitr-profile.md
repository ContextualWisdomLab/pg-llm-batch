...
2. Choose `backup_method="physical"` only for a crash-consistent base backup
   ...
   means you are not relying on a continuous WAL archive. It does **not** mean
   the base backup may omit backup-internal WAL. Crash-consistent restore still
   needs the WAL that `pg_basebackup -X stream` or `-X fetch` wrote into the
   backup.
3. Choose `backup_method="pitr"` only when a continuous WAL archive is already
   ...
   restore point, LSN, or `immediate` consistent state. `pitr` plus
   `immediate` stops at the first consistent state (`recovery_target =
   immediate`). It is not replay-to-end-of-archive. Default replay to the end
   of the archive is the absence of a recovery-target kind, which this binder
   does not encode.
...
- `pitr` + `immediate` is accepted as a consistent-state stop, not end-of-WAL;
- lone-surrogate profile text raises `PostgresPhysicalRecoveryError`, not
  `UnicodeEncodeError`;
