...
### PRD-T41 — Token-counting diagnostic confidentiality (Issue #131) — PLANNED

Prevent package-owned token-counting logs from copying arbitrary lower-layer PostgreSQL/Psycopg exception text. Protected main renders the generic `TokenCounter.count_tokens()` database exception directly at debug level even though PostgreSQL primary error messages may contain rejected values; the adjacent `nosemgrep` comment cannot make that path content-free. The bounded follow-up must prove the leak with a prompt/secret-like sentinel, replace lower-layer rendering with a finite package-owned failure category, preserve the distinct `UndefinedFunction` availability path and the no-Python-tokenizer-fallback contract, and avoid DSNs, SQL/binds, prompts, model text, credentials, dynamic exception names, or chained exception content in diagnostics. Implementation waits for ACTIVE-PR #87 or its protected successor because #87 currently owns `token_counter.py` validation and connection lifetime semantics; Issue #126 remains the separate provisioning-authority follow-up.

### PRD-T42 — Generic validation diagnostic confidentiality (Issue #132) — PLANNED

Make generic package validation failures confidentiality-safe instead of embedding rejected caller-controlled values into exported messages or structured diagnostics by default. The bounded follow-up must preserve actionable field/reason evidence while preventing prompts, credentials, DSNs, endpoint text, identifiers, or arbitrary object representations from leaking through `ValidationError` formatting, chaining, logging, or serialization. It must compose with the narrower active/source-specific confidentiality work in #104, #105, #71, #87, and #53 rather than duplicating their branches, and must include sentinel-based regressions showing rejected sensitive values are absent from both human-readable and structured error surfaces.

### PRD-T43 — Runtime config/secret provisioning authority (Issue #134) — PLANNED

Separate package-owned `com_config` / `com_secrets` schema provisioning and default seeding from ordinary runtime store construction. Protected main currently lets `PostgresConfigStore(...)` and `SecretStore(...)` execute `CREATE TABLE IF NOT EXISTS`, and config-store construction also inserts missing defaults, forcing ordinary application identities to retain schema-creation/bootstrap authority during routine reads and writes. The planned target moves DDL/default seeding into an explicit migration or bootstrap boundary, lets already-provisioned runtime roles operate without schema `CREATE` privilege, fails through bounded package-domain diagnostics when required schema capability is absent or incompatible, preserves fresh standalone initialization and embedded-host provisioning, and proves upgrade/rollback without overwriting operator configuration or secrets. Implementation waits for ACTIVE-PR #86, #87, and #89 or reviewed successors so typed configuration, resource ownership, and bootstrap-source semantics are composed from the exact protected result instead of raced.
