...
"""Regression contract for supported Core Metadata version authority."""
...
def _sdist_with_unsupported_metadata_version() -> bytes:
    """Return a bounded sdist declaring an unsupported Core Metadata major."""
    output = io.BytesIO()
    with tarfile.open(fileobj=output, mode="w:gz", format=tarfile.PAX_FORMAT) as archive:
        _add_regular_member(archive, f"{TOP_LEVEL}/PKG-INFO", UNSUPPORTED_CORE_METADATA)
        _add_regular_member(
            archive,
...
def _wheel_with_unsupported_metadata_version() -> bytes:
    """Return a bounded wheel declaring the same unsupported metadata major."""
    ...
    payloads = (
        ...
        (metadata_name, UNSUPPORTED_CORE_METADATA),
...
def test_release_verifier_rejects_unsupported_core_metadata_major(tmp_path: Path) -> None:
    """Reject reproducible artifacts whose Core Metadata major is unsupported."""
    sdist_bytes = _sdist_with_unsupported_metadata_version()
    wheel_bytes = _wheel_with_unsupported_metadata_version()
