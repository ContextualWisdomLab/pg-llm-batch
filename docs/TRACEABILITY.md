...
- **#202** — compatibility-aware `ValidationError` rejected-value confidentiality hardening; protected main still discloses arbitrary rejected values by default, so the candidate safe-default redaction and bounded `safe_value` opt-in remain ACTIVE-PR until integrated.
