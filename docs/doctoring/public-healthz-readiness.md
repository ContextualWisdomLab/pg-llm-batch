...
5. status-code and 404 behavior remain compatible;
6. `Cache-Control: no-store` is present; and
7. the parameterized transaction-local `statement_timeout` is set before the PostgreSQL health function executes.
...
No persistent data changes, database migrations, or external protocol state are introduced, so code **rollback** is mechanically straightforward. Reverting this change restores the former HTTP payload and therefore reintroduces the information-disclosure surface. Removing the transaction-local statement limit separately would also restore an unbounded connected-query path. If operational diagnosis is the reason for considering rollback, retain redaction and the bounded SQL contract and use the local diagnostic path instead.
