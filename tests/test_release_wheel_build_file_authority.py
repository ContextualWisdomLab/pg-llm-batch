...
"""Regression contract for build-system files forbidden by the wheel format."""
...
def _wheel_with_forbidden_build_file(name: str) -> bytes:
    """Return a valid wheel except for one forbidden build-system member."""
...
@pytest.mark.parametrize("forbidden_member", ["setup.py", "setup.cfg"])
def test_release_verifier_rejects_wheel_build_system_files(
    tmp_path: Path,
    forbidden_member: str,
) -> None:
    """Reject release evidence when a wheel carries setup.py or setup.cfg."""
    sdist_bytes = _matching_sdist()
    wheel_bytes = _wheel_with_forbidden_build_file(forbidden_member)
