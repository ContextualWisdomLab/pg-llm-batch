...
PR #321 owns only `README.md` and `docs/index.md` relative to #323. Its exact head, merge base, topology, checks, and review state are intentionally not pinned in this parent-owned baseline: recording a mutable descendant SHA here advances #323, immediately invalidates the recorded child ancestry, and creates a self-invalidating restack loop. Every run must read #321 live, require its merge base to equal the then-current #323 head, require only those two relative files, advance it only through ordinary non-force ancestry, and reacquire exact-child acceptance after any parent movement. Predecessor or parent GREEN is ancestry evidence only.
...
| Public commercial-license surface | Child #321 / live-evidence only | Read the live child against the then-current #323 head; preserve only `README.md` + `docs/index.md` through ordinary non-force reconciliation and reacquire exact-child CI/Release after every parent movement. |
