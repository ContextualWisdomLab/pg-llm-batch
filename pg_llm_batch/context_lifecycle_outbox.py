...
def _require_rls_application_role(cursor: Any) -> int:
    ...
    cursor.execute(
        ...
        "OR admitted_relation.relpersistence::pg_catalog.text OPERATOR(pg_catalog.<>) 'p' "
        "OR admitted_namespace.nspname OPERATOR(pg_catalog.<>) 'public' "
        "OR admitted_table_access_method.amname OPERATOR(pg_catalog.<>) 'heap' "
        "OR admitted_table_access_method.amtype::pg_catalog.text OPERATOR(pg_catalog.<>) 't' "
        "OR EXISTS ("
        "SELECT 1 FROM pg_catalog.pg_inherits AS live_outbox_inheritance "
        "WHERE live_outbox_inheritance.inhrelid OPERATOR(pg_catalog.=) admitted_relation.oid "
        "OR live_outbox_inheritance.inhparent OPERATOR(pg_catalog.=) admitted_relation.oid"
        ") "
        ...
        "JOIN pg_catalog.pg_namespace AS admitted_namespace "
        "ON admitted_namespace.oid OPERATOR(pg_catalog.=) admitted_relation.relnamespace "
        "JOIN pg_catalog.pg_am AS admitted_table_access_method "
        "ON admitted_table_access_method.oid OPERATOR(pg_catalog.=) admitted_relation.relam "
