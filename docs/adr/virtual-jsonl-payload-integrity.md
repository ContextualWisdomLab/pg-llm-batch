...
Protected `main` stores package-prepared upload content in `llm_batch_file_payloads.content JSONB`. `PostgresBatchOrchestrator._persist_payloads()` currently writes a JSON object with `text` containing the serialized JSONL bytes-as-text and `line_count` containing the assembled record count. `load_virtual_payload()` later reconstructs the provider upload from that row.

The current read boundary is permissive. `_normalize_payload_content()` returns `content.get("text", "")` for mappings, ignores `line_count`, accepts a bare string, and stringifies every other JSONB shape with `str(content)`. Consequently, corrupted, manually edited, migrated, or otherwise malformed package-owned persisted state can become an empty string or Python textual representation instead of being rejected, and contradictory line-count evidence is not detected. `BatchAPIClient.upload_jsonl()` currently resolves credentials before `_load_payload_bytes()` validates that persisted payload, so malformed local durable state can also cause unnecessary secret resolution before the operation fails. The defect is therefore a local persistence-integrity and effect-ordering problem before an external side effect, not merely a provider response-format problem.
...
- detect contradiction between persisted JSONL text and its package-written record count;
...
Partially useful but insufficient as the sole authority. It would protect one external effect, but other current or future consumers of `load_virtual_payload()` could still observe malformed durable state. Validation should live at the narrowest reusable persistence-reconstruction boundary, with the upload path retaining defensive credential/provider-effect ordering regressions.

### C. Add only a PostgreSQL `CHECK` constraint

Rejected as the initial universal remedy. A constraint can prevent some future malformed rows, but adding it to an existing installation without inspecting historical data can make upgrades fail or encourage destructive cleanup. It also cannot by itself validate every JSONL framing/content invariant or prove that `line_count` matches the records encoded in the text member.
...
1. `llm_batch_file_payloads.content` has one canonical package-owned representation for new writes: a JSON object containing the JSONL `text` and its `line_count`, matching the current package writer.
...
10. The implementation must keep package and container schema copies synchronized when schema changes are introduced.
...
A successful JSONB parse is not sufficient evidence that the value matches the package's application-level payload schema. PostgreSQL JSONB intentionally supports all JSON value kinds; pg-llm-batch must impose its narrower package contract explicitly. Likewise, a syntactically valid `line_count` is not sufficient if it contradicts the persisted text.
...
- valid canonical payload reconstruction with exact text/byte identity and matching `line_count`;
- malformed top-level list, number, boolean, null, unrelated object, missing/non-string text, missing/boolean/non-integer/negative line count, and text/count mismatch;
...
A read-validation implementation can be rolled back by reverting the validation code while retaining all durable rows. A database constraint can be rolled back only through its reviewed migration path and without deleting nonconforming evidence. A future ADR may supersede this decision only if it preserves explicit persisted-shape authority, `text`/`line_count` consistency, pre-provider fail-closed ordering, content-confidential diagnostics, existing-volume recovery, and deterministic acceptance evidence.
