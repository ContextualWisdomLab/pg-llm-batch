...
`SecretStore` requires Fernet encryption for every runtime construction and every newly persisted secret. A missing Fernet key, malformed key, unavailable `cryptography` package, or explicit `require_encryption=False` request fails with a fixed `ConfigError` before PostgreSQL connection acquisition. The `cryptography` package is a core install dependency, not an optional extra required only for tests. The historical `require_encryption` keyword is retained only so existing callers that explicitly pass `True` do not break at the call boundary; it is not a policy switch and cannot re-enable reversible Base64 persistence. Direct SQL `INSERT` into `com_secrets` must set `is_encrypted` explicitly to `TRUE` and store Fernet ciphertext. The column default remains `FALSE` so an omitted flag stays detectable as unencrypted instead of being silently marked encrypted.
