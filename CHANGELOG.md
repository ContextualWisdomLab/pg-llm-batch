...
- Added fail-closed legacy PostgreSQL extension retirement for existing volumes: the operator migration blocks on package or operator cron authority and retired helper signatures, uses one bounded transaction with `DROP EXTENSION ... RESTRICT`, preserves `gateway_retrieval_logs`, forbids `CASCADE`, documents recovery and rollback, and proves idempotent replay in a live container.
