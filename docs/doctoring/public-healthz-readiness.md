...
Readiness database targeting is explicit and fail-closed. An **invalid Postgres DSN**—including a non-string value, an empty string, or a whitespace-only string—returns a body-safe local database failure **before psycopg** connection selection and therefore before libpq can apply ambient/default target resolution. The invalid value itself is not reflected in the diagnostic. A valid non-empty string is passed through unchanged; this boundary rejects invalid authority rather than trimming, stringifying, or silently selecting another database.
...
The listener is secure by default when invoked directly. `python -m pg_llm_batch serve-healthz` binds to loopback **`127.0.0.1`** unless an operator supplies `--host`. A deployment that intentionally exposes readiness beyond the local host must therefore make that network decision **explicit**. The bundled **container** is one such deployment boundary: its Docker `CMD` explicitly supplies `--host 0.0.0.0` so container port publishing and orchestration probes continue to work without making the general CLI default remotely reachable.
...
Threaded request handling needs its own availability boundary. Python documents that `ThreadingMixIn` dispatches requests in threads. An admission-free thread-per-request endpoint can therefore exchange serial blocking for resource pressure when many connections arrive together. MITRE describes uncontrolled resource consumption under **CWE-400** and specifically identifies allocation without limits or throttling as **CWE-770**. The package consequently applies a fixed concurrent admission ceiling before any new health worker can reach PostgreSQL.

This change is **not authentication**. It neither hides the existence of the endpoint nor authorizes callers. An operator who needs to prevent untrusted network access must still use deployment controls such as private networking, ingress rules, service-mesh policy, firewall policy, or an authenticated gateway where appropriate. The package's responsibility is narrower: if a caller can reach readiness, the response does not unnecessarily disclose local diagnostic detail or obtain unbounded in-process readiness concurrency. Likewise, an explicit `--host 0.0.0.0` opt-in only selects a listener address; it does not authenticate clients or make broad exposure safe by itself.

## Probe, query, admission, and caching behavior
...
The listener must not make a bounded but delayed health query a server-wide serial queue. The standalone endpoint therefore instantiates `_ThreadingHealthHTTPServer`, a narrow combination of `ThreadingMixIn` and `HTTPServer`, with daemon request threads. Independent readiness probes can start concurrently while another probe is waiting on its own bounded check. The same redaction, status, header, database timeout, and endpoint-path contracts apply to every request; concurrency does not turn a failed check into success or share diagnostic details between callers.

The server accepts a **maximum of 32 admitted readiness requests** at once. Admission uses a non-blocking bounded semaphore before `ThreadingMixIn` allocates another worker. If all slots are in use, an excess accepted connection is **closed before a worker thread or database check starts**; it is not allowed to consume another health worker or PostgreSQL check. Each slot is **released after request completion or thread-start failure**, including exceptional request completion through the request-thread `finally` path. This is an in-process concurrency/resource ceiling, not a per-origin quota or deployment rate limiter.
...
The change does not alter PostgreSQL schema, migrations, provider requests, credentials, batch state, or MSA integration. A direct host CLI invocation remains local-only by default because `serve-healthz` binds to `127.0.0.1`. Docker and Compose may continue to use `/healthz` because the bundled container explicitly opts into `0.0.0.0` inside the container before the deployment layer decides whether and how to publish that port. Remote or production exposure is therefore an explicit deployment decision rather than an accidental inheritance from a permissive CLI default. Embedded hosts can call `check_health()` for a trusted local operator surface, while external readiness consumers receive the redacted projection. The local CLI keeps diagnostic detail but returns not-ready when required-component identity is missing or duplicated, so detailed troubleshooting cannot override ambiguous required readiness evidence.

There is no new dependency on `contextual-orchestrator`, `naruon`, or another CWL service. The same package works independently and as an imported MSA component. The timeout is established per health transaction, so standalone and embedded callers do not need a PostgreSQL configuration migration. Concurrent request scheduling and the 32-request ceiling remain internal to the blocking `serve_healthz()` process entrypoint and require no client or deployment-protocol migration.
...
8. inherited `BaseHTTPRequestHandler` response behavior cannot reintroduce a stdlib/Python `Server header`;
9. invalid Postgres DSN values, including non-string and whitespace-only inputs, fail before psycopg/libpq target selection;
10. the parameterized transaction-local `statement_timeout` is set before the PostgreSQL health function executes;
11. the instantiated listener includes `ThreadingMixIn`, so independent readiness probes are not serialized behind one delayed request;
12. 33 simultaneously offered requests allocate at most 32 workers and close the excess request before database work;
13. both successful request completion and thread-start failure return the admission slot so capacity cannot leak across requests; and
14. the direct CLI defaults to `127.0.0.1` while the bundled container requests `0.0.0.0` explicitly.
...
If `/healthz` reports `503`, use trusted **local operator diagnostics** such as `python -m pg_llm_batch health`, database administration tooling, and protected logs. If local diagnostics report `invalid Postgres DSN`, correct the explicit configured DSN; do not rely on ambient libpq defaults as a recovery shortcut. If local diagnostics show duplicate required-component rows, investigate the `pg_llm_batch_health_check()` contract or deployment schema rather than accepting either duplicate as authoritative. If local diagnostics contain an unrecognized component name, keep it on the trusted operator surface unless an explicit public-contract change reviews and adds that identity to the required set. If local diagnostics indicate malformed readiness data, treat that as a contract or integration defect rather than coercing the value into a boolean. If local diagnostics indicate a statement-timeout cancellation, investigate the health function's locks, database load, extension availability, and query execution rather than increasing or removing the timeout as a first response. If probes are refused at the 32-request ceiling, investigate probe fan-out, retry frequency, ingress/service-mesh queues, file-descriptor pressure, database capacity, and whether probes are being exposed more broadly than intended; do not simply remove the ceiling. If probes queue at the deployment edge, investigate ingress, service-mesh, runner, socket, and resource limits before reverting concurrent request handling to a serial listener. If a remote probe cannot reach a host CLI process after this change, decide explicitly whether that process should be remotely reachable and, only when intended, supply a reviewed `--host` value such as `0.0.0.0` together with appropriate ingress/firewall controls. Do not restore a remotely reachable CLI default merely to bypass deployment configuration. Do not add detailed exception text, arbitrary local component identities, or a runtime-identifying `Server header` back to the public endpoint as an incident shortcut.

## Rollback

No persistent data changes, database migrations, or external protocol state are introduced, so code **rollback** is mechanically straightforward. Reverting this change restores the former HTTP payload and therefore reintroduces the information-disclosure surface. Reverting strict readiness validation would also reintroduce truth-coercion and duplicate-row false-ready risk. Reverting database-target validation would allow invalid DSN values to reach psycopg/libpq and could silently re-enable ambient/default target selection. Reverting the component-name filter would allow newly introduced local component identities to cross the public boundary without explicit review. Restoring the default `BaseHTTPRequestHandler.send_response()` behavior would reintroduce the stdlib/Python `Server header` and expose the runtime `Python version`. Removing the transaction-local statement limit separately would restore an unbounded connected-query path. Removing `ThreadingMixIn` would restore serial request handling and allow one delayed check to block independent readiness probes. Removing the 32-request admission ceiling while keeping threaded serving would reopen unbounded worker/database allocation under connection flood. Reverting the loopback CLI default would also make direct invocations listen beyond the local host again unless every caller supplied an explicit host. If operational diagnosis is the reason for considering rollback, retain redaction, the fixed required-component allow-list, the non-coercive public projection, duplicate-row rejection, runtime-fingerprint suppression, the explicit listener-exposure boundary, bounded concurrent request handling, explicit DSN authority, and the bounded SQL contract and use the local diagnostic path instead.
