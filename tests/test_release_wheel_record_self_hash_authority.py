...
"""Regression contract forbidding wheel RECORD from hashing itself."""
...
def _wheel_with_hashed_record_self_row() -> bytes:
    """Return a wheel whose RECORD row incorrectly carries a secure self hash."""
    ...
    forbidden_self_hash = _urlsafe_digest(b"RECORD cannot contain its own hash")
    rows = [
        ...
        f"{PACKAGE_MEMBER},sha256={_urlsafe_digest(package_payload)},{len(package_payload)}",
        f"{record_name},sha256={forbidden_self_hash},",
    ]
    record = ("\n".join(rows) + "\n").encode("utf-8")
...
def test_release_verifier_rejects_record_self_hash(tmp_path: Path) -> None:
    """Require a wheel RECORD self-row to keep its hash field empty."""
    sdist_bytes = _matching_sdist()
    wheel_bytes = _wheel_with_hashed_record_self_row()
