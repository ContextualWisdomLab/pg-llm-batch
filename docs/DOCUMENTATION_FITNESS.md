...
| Technical requirements | MISSING | PRESENT-CURRENT | Added as `docs/product/TRD.md`; #106 operation-span failure-status semantics are explicitly ACTIVE-PR rather than shipped. |
...
| Threat model | MISSING | PRESENT-CURRENT | Added as `docs/THREAT_MODEL.md`; separates package and host responsibilities, classifies #105 caller-owned error-evidence alias drift without immutable-audit overclaims, and classifies #106 description-free operation-span Error status with telemetry-failure isolation. |
| Data governance / privacy | PARTIAL | PRESENT-CURRENT | Protected main contains privacy-sensitive persistence and bounded telemetry/diagnostic rules but no canonical package-vs-host governance authority. `docs/DATA_GOVERNANCE.md` now classifies content/credentials/metadata, preserves purpose-bound utility instead of blanket masking, labels #53 tenant isolation, #71 provider-error confidentiality, and #105 structured error evidence ACTIVE-PR, and makes retention/erasure/backup/residency host-owned unless a future accepted contract changes that boundary. |
| Test strategy | PARTIAL | PRESENT-CURRENT | Existing CI/pyproject encoded gates but no explanatory canonical strategy. |
| Operability / recovery | PARTIAL | PRESENT-CURRENT | README and doctoring covered fragments; a canonical runbook is added and now includes scheduler/control-plane failure recovery, user redirection, and silent-completion handling without conflating them with repository failure. |
...
| Traceability | MISSING | PRESENT-CURRENT | Added requirement/decision -> source/schema/test/evidence mapping, including scheduler failure recovery, the compact prompt source, public runtime hardening, structured error evidence authority, and the #106 operation span error status overlay. |
