...
def test_acquisition_licensing_authority_is_discoverable_and_consistent() -> None:
    ...
    assert "[Licensing and IP](docs/LICENSING_AND_IP.md)" in readme
    assert "Licensing / IP / third-party notices" in fitness
    assert "docs/LICENSING_AND_IP.md" in traceability
    assert "licensing" in release.lower() and "notice" in release.lower()
