...
- Keep row-level security enabled and forced. Runtime admission must reject
  ...
  `CREATEDB`. That executable-principal check is transitive: once a caller can
  enter a user-schema `SECURITY DEFINER`, admission must recursively follow any
  further user-schema `SECURITY DEFINER` that the discovered owner can execute
  through schema `USAGE` plus routine `EXECUTE`, using a cycle-safe owner closure.
  A safe outer owner therefore cannot hide a dangerous inner definer that the
  caller cannot execute directly. PostgreSQL lets the membership administrator
  grant a role onward even when that administrator's own membership is `INHERIT
  FALSE, SET FALSE`; the recipient can then use the granted selectable path after
  a security-definer call returns. `SECURITY DEFINER` executes with its owner's
  privileges rather than the caller's privileges. Admission must re-prove the
  sole canonical tenant policy's command, role scope, permissive mode,
  `USING`/`WITH CHECK` predicates, and reviewed catalog dependencies before tenant
  binding or outbox SQL. Direct runtime `CREATEDB` and `CREATEROLE` are
  ...
  grant options, authority-bearing role administration, and executable privileged
