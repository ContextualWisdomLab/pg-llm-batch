...
"""Regression contract for case-insensitive source-distribution member identity."""
...
SDIST_PACKAGE_MEMBER = f"{TOP_LEVEL}/{PACKAGE_MEMBER}"
CASE_COLLIDING_MEMBER = f"{TOP_LEVEL}/pg_llm_batch/__INIT__.PY"
CORE_METADATA = (
...
def _sdist_with_case_insensitive_collision() -> bytes:
    """Return a bounded sdist with two names that shadow on case-insensitive filesystems."""
    output = io.BytesIO()
    with tarfile.open(fileobj=output, mode="w:gz", format=tarfile.PAX_FORMAT) as archive:
        ...
        _add_regular_member(
            archive,
            SDIST_PACKAGE_MEMBER,
            b"__version__ = '0.1.0'\n",
        )
        _add_regular_member(
            archive,
            CASE_COLLIDING_MEMBER,
            b"__version__ = '9.9.9'\n",
        )
...
def test_release_verifier_rejects_case_insensitive_sdist_member_collision(
    tmp_path: Path,
) -> None:
    """Reject distinct TAR names that would shadow each other by ASCII case."""
    assert SDIST_PACKAGE_MEMBER != CASE_COLLIDING_MEMBER
    assert SDIST_PACKAGE_MEMBER.casefold() == CASE_COLLIDING_MEMBER.casefold()

    sdist_bytes = _sdist_with_case_insensitive_collision()
