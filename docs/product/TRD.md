...
### TRD-H10 — Strict provider progress/control values (#71 / Issue #98) — ACTIVE-PR

ACTIVE-PR #71 now owns the bounded provider batch-status/progress validation originally tracked by Issue #98. A successful batch-status response requires a non-empty status string. `request_counts`, when present, must be an object, and `total`, `completed`, and `failed` must each be exact non-negative integers; booleans, strings, negative values, `NaN`, `Infinity`, and other non-conforming control values fail closed. The accepted invariant is `completed + failed <= total`. Malformed evidence raises fixed body-free `InvalidBatchStatusPayload` diagnostics rather than raw runtime failures or provider-controlled detail. The implementation preserves the existing control-response byte bound, strict UTF-8/JSON parsing, retry/no-POST-replay boundary, response-handoff semantics, and provider-error confidentiality. This remains ACTIVE-PR until #71 or a reviewed successor reaches protected main and receives fresh exact-source/integrated acceptance; Issue #98 stays open until that boundary is proven.
