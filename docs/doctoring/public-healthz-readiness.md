...
10. the instantiated listener includes `ThreadingMixIn`, so independent readiness probes are not serialized behind one delayed request;
11. 33 simultaneously offered requests allocate at most 32 workers and close the excess request before database work;
12. both successful request completion and thread-start failure return the admission slot so capacity cannot leak across requests; and
13. the direct CLI defaults to `127.0.0.1` while the bundled container requests `0.0.0.0` explicitly.
