...
| Bounded GET retry/TLS/response-handoff classification | ACTIVE-PR #71 | `pg_llm_batch/batch_api_client.py` hardening | branch CI/security/SAST evidence is #71-specific; final exact-source/review gates remain required | TRD, THREAT_MODEL, ADR index, DATA_GOVERNANCE |
