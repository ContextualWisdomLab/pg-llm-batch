...
def bind_postgres_physical_recovery_profile(
    *,
    ...
    restore. ``wal_archive_required=False`` means no continuous WAL archive; a
    crash-consistent restore still needs backup-internal WAL from
    ``pg_basebackup -X stream`` or ``-X fetch``. ``pitr`` plus ``immediate`` is
    ``recovery_target=immediate``, not replay to the end of the archive.
    Optional ``rpo_seconds`` and ``rto_seconds`` are deployer-selected
...
def parse_postgres_physical_recovery_profile(
    ...
    Lone-surrogate or other UTF-8 encoding failures raise
    ``PostgresPhysicalRecoveryError`` with a content-free JSON diagnostic so
    hosts catching only that type do not observe a leaked ``UnicodeEncodeError``.
    """
