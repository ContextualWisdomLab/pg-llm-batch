...
"""Regression contract for Windows-reserved source-distribution member names."""
...
def _sdist_with_windows_reserved_member(reserved_name: str) -> bytes:
    """Return a bounded sdist containing one Windows-reserved path component."""
    output = io.BytesIO()
    with tarfile.open(fileobj=output, mode="w:gz", format=tarfile.PAX_FORMAT) as archive:
        ...
        _add_regular_member(
            archive,
            f"{TOP_LEVEL}/{PACKAGE_MEMBER}",
            b"__version__ = '0.1.0'\n",
        )
        _add_regular_member(
            archive,
            f"{TOP_LEVEL}/pg_llm_batch/{reserved_name}",
            b"reserved-name payload\n",
        )
...
@pytest.mark.parametrize("reserved_name", ["CON", "NUL.txt"])
def test_release_verifier_rejects_windows_reserved_sdist_member_name(
    tmp_path: Path,
    reserved_name: str,
) -> None:
    """Reject TAR members whose path components map to Windows device names."""
    sdist_bytes = _sdist_with_windows_reserved_member(reserved_name)
