            ...
            try:
                with self._attacker_connection.cursor() as attacker:
                    ...
                    attacker.execute(
                        ...
                        "'tenant-a', 'admission-schema-race', 'batch.lifecycle.observed', "
                        "%s, %s, %s, %s, 'observed', "
...
assert cursor.attack_blocked, "runtime did not retain public-schema identity through SELECT"
assert not cursor.attack_committed, "hostile schema replacement committed after admission"
assert loaded is None, "runtime read from a post-admission replacement schema"
PY
