...
"""Regression contract for deprecated wheel RECORD signature members."""
...
def _wheel_with_deprecated_signature(signature_name: str) -> bytes:
    """Return a canonical wheel except for one deprecated signature member."""
    metadata_name = f"{DIST_INFO}/METADATA"
    wheel_name = f"{DIST_INFO}/WHEEL"
    record_name = f"{DIST_INFO}/RECORD"
    signature_member = f"{DIST_INFO}/{signature_name}"
    ...
    signature_payload = b"deprecated in-archive signature fixture\n"
    recorded_payloads = (
    ...
    rows = [
        ...
        for member_name, payload in recorded_payloads
    ]
    ...
    with zipfile.ZipFile(output, mode="w", compression=zipfile.ZIP_STORED) as archive:
        members = (
            *recorded_payloads,
            (record_name, record),
            (signature_member, signature_payload),
        )
...
def test_release_verifier_rejects_deprecated_wheel_record_signatures(
    tmp_path: Path,
    signature_name: str,
) -> None:
    """Reject newly produced wheels carrying deprecated RECORD signatures."""
    sdist_bytes = _matching_sdist()
    wheel_bytes = _wheel_with_deprecated_signature(signature_name)
