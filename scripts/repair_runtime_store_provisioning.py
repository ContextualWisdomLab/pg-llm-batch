#!/usr/bin/env python3
"""Apply the bounded runtime-store provisioning repair on its owned branch."""
...
def _replace_runtime_blocks(source: str) -> str:
    """Replace constructor-owned provisioning with bounded capability checks."""
    ...
    source = source[:secret_start] + SECRET_RUNTIME_BLOCK + source[secret_end:]
    return source


def _seed_schema(schema: str) -> str:
    """Seed built-in configuration once at the explicit schema boundary."""
...
def main() -> int:
    """Apply the source and mirrored-schema repair deterministically."""
