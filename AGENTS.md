...
- The standalone `serve-healthz` CLI must default to loopback `127.0.0.1`; any deployment that intentionally listens beyond the local host must opt in explicitly. The bundled container is such a deployment boundary and must request `--host 0.0.0.0` explicitly rather than inheriting a permissive CLI default.
