...
"""Regression contract for safe source-distribution link targets."""
...
UNSAFE_LINKS = (
    pytest.param(tarfile.SYMTYPE, "../../escape.py", id="symlink-outside-root"),
    pytest.param(tarfile.LNKTYPE, "/escape.py", id="hardlink-absolute-target"),
)
...
def _sdist_with_unsafe_link(link_type: bytes, link_name: str) -> bytes:
    """Return a bounded sdist containing one link that escapes extraction authority."""
    output = io.BytesIO()
    with tarfile.open(fileobj=output, mode="w:gz", format=tarfile.PAX_FORMAT) as archive:
        ...
        member = tarfile.TarInfo(f"{TOP_LEVEL}/linked.py")
...
@pytest.mark.parametrize(("link_type", "link_name"), UNSAFE_LINKS)
def test_release_verifier_rejects_sdist_links_outside_extraction_root(
    tmp_path: Path,
    link_type: bytes,
    link_name: str,
) -> None:
    """Reject reproducible sdists whose links resolve outside the destination."""
    sdist_bytes = _sdist_with_unsafe_link(link_type, link_name)
