...
| Candidate-to-production construction | Active Draft | Exact-version lazy construction, pre-execution distribution/import-origin binding, and explicit service-file composition are implemented behind `PostgresDriverPort`; prove the unchanged final runtime graph after switching the single selector and manifest/lock together. |
