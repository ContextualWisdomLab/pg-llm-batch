...
def test_load_returns_none_and_uses_tenant_qualified_key(
...
def test_load_in_transaction_uses_caller_cursor_without_commit(
    database: FakeDatabase,
) -> None:
    ...
    assert (
        store.load_in_transaction(cursor, "worker-a", "batch-1", "default")
        == checkpoint()
    )
...
def test_load_rejects_noncanonical_identifiers_before_database(
    database: FakeDatabase,
) -> None:
...
def test_save_creates_and_idempotently_repeats_checkpoint(
    database: FakeDatabase,
) -> None:
...
def test_save_in_transaction_does_not_commit_caller_work(
    database: FakeDatabase,
) -> None:
...
def test_save_handles_same_checkpoint_initial_insert_race(
    database: FakeDatabase,
) -> None:
    ...
    assert (
        sum(statement.startswith("SELECT schema_version") for statement in statements)
        == 2
    )


def test_save_rejects_disappearing_insert_conflict_row(
    database: FakeDatabase,
) -> None:
...
def test_save_fails_closed_on_different_initial_insert_race(
    database: FakeDatabase,
) -> None:
...
def test_save_requires_missing_expected_checkpoint_to_remain_missing(
    database: FakeDatabase,
) -> None:
...
def test_save_rejects_stale_or_forked_expected_checkpoint(
    database: FakeDatabase,
) -> None:
...
def test_save_rejects_wrong_types_and_mismatched_expected_identity(
    database: FakeDatabase,
) -> None:
...
def test_apply_schema_uses_explicit_migration(
    database: FakeDatabase,
    tmp_path: Any,
) -> None:
