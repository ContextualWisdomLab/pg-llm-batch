...
"""Regression contract for safe wheel archive-member path authority."""
...
UNSAFE_MEMBERS = ("../escape.py", "/absolute.py")
...
def _wheel_with_unsafe_member(member_name: str) -> bytes:
    """Return a bounded wheel whose RECORD correctly hashes one unsafe member path."""
    ...
    unsafe_payload = b"raise RuntimeError('must never install outside wheel root')\n"
    rows = [
        f"{member_name},sha256={_record_digest(unsafe_payload)},{len(unsafe_payload)}",
...
def test_release_verifier_rejects_unsafe_wheel_member_paths(
    tmp_path: Path,
    unsafe_member: str,
) -> None:
    """Reject absolute or parent-traversing members despite valid RECORD hashes."""
    sdist_bytes = _matching_sdist()
    wheel_bytes = _wheel_with_unsafe_member(unsafe_member)
