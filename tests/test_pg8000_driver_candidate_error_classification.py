...
class _DatabaseError(Exception):
    """Stand in for pg8000 1.31.5's server-error DB-API class."""
...
def test_candidate_classifier_rejects_malformed_database_error_authority() -> None:
    module = _dbapi_module()
    module.DatabaseError = "DatabaseError"

    with pytest.raises(
        Pg8000CandidateErrorEvidenceError,
        match="DatabaseError authority is invalid",
    ):
        is_pg8000_candidate_undefined_function(
            _DatabaseError({"C": "42883"}),
            dbapi_module=module,
