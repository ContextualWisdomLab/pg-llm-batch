...
2. **Admission to read relation object.** A table owner can wait until live admission returns safe, rename or replace the admitted relation, and make the later `SELECT` resolve a relation object the catalog proof never admitted unless the read path pulls its relation lock ahead of admission. This is the relation-identity TOCTOU that the admission-to-read fence must close.
