...
"""Regression contract for exact wheel RECORD row shape."""
...
def _wheel_with_record_row(package_row: str) -> bytes:
    """Return a bounded wheel whose package RECORD row has caller-selected shape."""
...
@pytest.mark.parametrize("row_shape", ["missing_size", "extra_field"])
def test_release_verifier_rejects_non_three_field_record_rows(
    tmp_path: Path,
    row_shape: str,
) -> None:
    """Require every wheel RECORD row to contain exactly path, hash, and size."""
    package_payload = b"__version__ = '0.1.0'\n"
    digest = _record_digest(package_payload)
    if row_shape == "missing_size":
        package_row = f"{PACKAGE_MEMBER},sha256={digest}"
    else:
        package_row = f"{PACKAGE_MEMBER},sha256={digest},{len(package_payload)},extra"

    sdist_bytes = _matching_sdist()
    wheel_bytes = _wheel_with_record_row(package_row)
