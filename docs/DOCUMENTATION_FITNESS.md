...
| Product PRD | MISSING | PRESENT-CURRENT | Added as `docs/product/PRD.md`; file maturity is ACTIVE-PR until merge, and PRD-T31/#121 plus PRD-T32/Issue #122 remain PLANNED rather than shipped. |
| Technical requirements | MISSING | PRESENT-CURRENT | Added as `docs/product/TRD.md`; #106 operation-span failure-status and #119 PostgreSQL logging privacy semantics are explicitly ACTIVE-PR, while PostgreSQL image reproducibility Issue #118, container-native logging Issue #120, provider-secret encryption Issue #121, package-owned PostgreSQL wait budgets Issue #122, runtime tokenizer provisioning Issue #126, and durable lifecycle field validation Issue #127 remain PLANNED rather than shipped. |
| Public API / CLI / schema / version contract | PARTIAL | PRESENT-CURRENT | Added as `docs/product/API_CONTRACT.md`; explicitly separates protected-main exports/commands/schema from ACTIVE-PR overlays and defines Semantic Versioning/deprecation rules, including #104 runtime typing, #105 structured exception snapshot semantics, and the PLANNED #128 `GatewayCredentials` representation boundary without shipped, immutability, or arbitrary-serialization overclaims. |
...
| Threat model | MISSING | PRESENT-CURRENT | Added as `docs/THREAT_MODEL.md`; separates package and host responsibilities, classifies #105 caller-owned error-evidence alias drift, #106 description-free operation-span Error status, #119 persistent-vs-volatile PostgreSQL query-text exposure, #126 runtime/provision authority, #127 durable lifecycle provider-field risk, and PLANNED #128 provider credential representation disclosure without overclaiming shipped controls. |
...
| Provider-secret storage policy | PARTIAL | PRESENT-CURRENT | Protected main supports optional Fernet but deliberately falls back to Base64 obfuscation with no key. Issue #121 is PLANNED to make encryption-required operation explicit, migrate existing unencrypted rows safely, and define bounded key rotation; `docs/adr/provider-secret-encryption-policy.md` is the canonical decision and does not call Base64 encryption. |
| Provider credential representation | PARTIAL | PRESENT-CURRENT | Protected-main `GatewayCredentials` is a default dataclass whose ordinary representation includes secret-bearing `api_key`. **PLANNED #128** removes that disclosure from `repr()`/`str()` after #71 settles while preserving direct credential access; general serialization remains separate caller authority. |
| Generic validation diagnostic confidentiality | PARTIAL | PRESENT-CURRENT | Protected-main generic package validation formatting can still expose rejected caller values through messages/details at call sites not yet covered by source-specific confidentiality repairs. **PLANNED #132 generic validation diagnostic confidentiality** requires a finite package-owned field/reason vocabulary and sentinel regressions across prompts, credentials, DSNs, endpoint text, identifiers, and arbitrary object representations without weakening actionable diagnostics. |
| Runtime config/secret provisioning authority | PARTIAL | PRESENT-CURRENT | Protected-main `PostgresConfigStore` / `SecretStore` can couple runtime use with schema/default provisioning authority. **PLANNED #134 runtime config/secret provisioning authority** separates explicit provisioning from least-privilege runtime access after ACTIVE-PR #86/#87/#89 settle, preserving `com_config` and `com_secrets` values across fresh install, upgrade, rollback, and recovery. |
| Test strategy | PARTIAL | PRESENT-CURRENT | Existing CI/pyproject encoded gates but no explanatory canonical strategy. |
| Operability / recovery | PARTIAL | PRESENT-CURRENT | README and doctoring covered fragments; a canonical runbook is added and now includes scheduler/control-plane recovery. Issue #120 tracks container-native storage-bounded PostgreSQL logging, while Issue #122 tracks finite package-owned PostgreSQL connection timeout and operation-class statement/lock wait budgets after overlapping DB ownership surfaces settle. |
...
| Traceability | MISSING | PRESENT-CURRENT | Added requirement/decision -> source/schema/test/evidence mapping, including scheduler recovery, #116/#117 argv privacy gaps, #118 PostgreSQL image reproducibility, #119/#120 logging boundaries, #121 secret-encryption policy, #122 package-owned PostgreSQL wait-budget planning, #125 durable lifecycle failure diagnostics, #126 runtime tokenizer provisioning, #127 durable lifecycle provider fields, #128 provider credential representation confidentiality, #132 generic validation diagnostic confidentiality, and #134 runtime config/secret provisioning authority. |
| Product ADR index and foundational decisions | MISSING | PRESENT-CURRENT | `docs/adr/README.md` indexes detailed protected-main foundation records plus planned security decisions such as Issue #121; the new records themselves remain ACTIVE-PR documentation until #93 integrates. |
...
| PostgreSQL wait budgets | PARTIAL | PRESENT-CURRENT | Protected main has ordinary package-created PostgreSQL connections and no general package-owned finite statement/lock timeout policy outside the readiness-specific #70 overlay. Issue #122 is PLANNED to add finite connection acquisition and operation-class database wait budgets without leaking DSNs/query content or silently changing caller-owned transactions. |
| Runtime tokenizer provisioning authority | PARTIAL | PRESENT-CURRENT | Protected main can attempt `CREATE EXTENSION` for `pg_tiktoken` during ordinary **runtime** token counting. Issue #126 is **PLANNED** to separate reviewed **provision** authority from read-only least-privilege runtime counting, with bounded failure semantics and no Python tokenizer fallback. |
| Durable lifecycle failure diagnostics | PARTIAL | PRESENT-CURRENT | Protected-main durable lifecycle recovery can retain dynamic exception class names and cause/context detail. Issue #125 is PLANNED to replace provider/runtime-derived diagnostic vocabulary with a finite package-owned durable lifecycle recovery contract while preserving reconciliation evidence and content confidentiality. |
| Durable lifecycle provider fields | PARTIAL | PRESENT-CURRENT | Protected-main `batch_status` and `batch_endpoint` are durable provider-derived fields without the final **finite** reviewed domain. Issue #127 is **PLANNED** to validate both before persistence, make cancelling/terminal semantics explicit, and keep provider endpoint evidence out of credential/destination authority. |
...
- PostgreSQL `pg_tiktoken`-backed token counting and batch assembly. Protected-main runtime initialization may still attempt extension provisioning; that authority boundary is PARTIAL pending Issue #126.
- Disk-free JSONL persistence and reconstruction through PostgreSQL tables.
- OpenAI-compatible Files/Batches client with bounded request timeout, bounded control-plane JSON, bounded provider-file downloads, destination/resource validation, and bounded retries for the currently reviewed GET status set. Protected-main error translation at this baseline must not be described as already satisfying the stronger #71 provider-error confidentiality overlay. `GatewayCredentials` is also a public protected-main value object, but its generated representation can include `api_key`; that representation-confidentiality boundary remains PARTIAL pending Issue #128.
- `DurableBatchAPIClient` with database-owned remote lifecycle observation ordering and persistence in `llm_remote_batch_jobs`; provider-derived `batch_status` and `batch_endpoint` validation remains PARTIAL pending Issue #127.
- Database-backed configuration and secret storage (`com_config`, `com_secrets`) with optional Fernet encryption; absent-key Base64 storage is a deliberate local/development obfuscation fallback, not an encryption guarantee.
...
- Issue #121 — planned explicit **provider-secret encryption** deployment policy. An encryption-required mode must fail before persistence without usable Fernet authority, classify Base64 as development obfuscation rather than encryption, migrate existing unencrypted rows safely, define bounded key rotation/recovery, and preserve host credential-provider interoperability after #85/#87/#89 settle.
- Issue #122 — planned finite package-owned **PostgreSQL connection timeout** plus operation-class-specific statement/lock wait budgets after #87/#89/#53 settle. The design must preserve caller-owned transaction authority, rollback timed-out package transactions, and emit bounded diagnostics without DSNs, SQL, bind values, credentials, prompts, or provider content.
- Issue #125 — planned **durable lifecycle** failure-diagnostic hardening after #53 settles the same lifecycle surface. Replace dynamic exception class names and provider/runtime-derived text with a finite package-owned recovery vocabulary, preserve phase/operation/order/validated identity needed for reconciliation, bound cause/context retention, and keep credentials/provider bodies/request content out of durable diagnostics.
- Issue #126 — planned separation of `pg_tiktoken` extension **provision** authority from ordinary **runtime** token counting after #87 settles the same initialization/connection surface. Runtime counting must be read-only and least-privilege, must fail closed on missing capability, and must not activate a Python tokenizer fallback.
- Issue #127 — planned **finite** validation for durable provider `batch_status` and `batch_endpoint` after #53 settles the lifecycle surface, with explicit cancelling/terminal semantics, bounded relative endpoint grammar, fail-before-write behavior, and no provider endpoint authority over credential-bearing destinations.
- Issue #128 — planned **provider credential representation** confidentiality after #71 settles the provider-client surface. `GatewayCredentials` must preserve constructor/direct-attribute compatibility while ordinary `repr()`/`str()` omit `api_key`; this does not make arbitrary serialization, reflection, or caller-defined serializers safe by implication.
- Issue #132 — planned **generic validation diagnostic confidentiality** after overlapping source-specific validation owners settle. Package-domain validation must retain actionable field/reason evidence while proving rejected prompts, credentials, DSNs, endpoint text, identifiers, and arbitrary object representations do not become exported diagnostic content, cause/context, logs, or serialized package evidence.
- Issue #134 — planned **runtime config/secret provisioning authority** after ACTIVE-PR #86/#87/#89 settle. Explicit provisioning owns schema creation/default seeding so already-provisioned least-privilege runtime roles can use `com_config` and `com_secrets` without schema `CREATE`; missing/incompatible schema fails through bounded diagnostics and upgrade/rollback preserves operator values.
...
20. **Base64 is not an encryption policy.** Protected-main Base64 storage without a Fernet key is development obfuscation. PLANNED Issue #121 must make encryption-required operation explicit, fail before persistence on missing encryption authority, and provide migration/key-rotation recovery rather than silently treating a warning as a security boundary.
21. **Database resource closure is not a wait budget.** Deterministic connection cleanup does not prove bounded acquisition, statement, or lock wait time. PLANNED Issue #122 must compose finite package-owned database waits with #87/#89/#53 without rewriting caller authority or exposing database content in timeout diagnostics.
22. **Dynamic exception identity is not durable lifecycle evidence.** PLANNED Issue #125 must keep recovery diagnostics finite and package-owned; dynamic exception class names, provider text, credentials, request content, and unbounded cause/context chains are not durable reconciliation authority.
23. **Runtime counting is not provisioning authority.** PLANNED Issue #126 must remove ordinary runtime dependence on extension-install privilege: `pg_tiktoken` provision belongs to reviewed install/migration/bootstrap, while runtime counting is read-only, least-privilege, and fails closed without Python tokenizer fallback.
24. **Provider lifecycle evidence is not destination authority.** PLANNED Issue #127 must validate `batch_status` and `batch_endpoint` against finite/bounded policy before durability and must never let provider-returned endpoint text select a credential-bearing destination.
25. **Safe credential representation is not arbitrary serialization.** PLANNED #128 must keep `GatewayCredentials.api_key` out of ordinary `repr()`/`str()` and package-owned rendering without implying that direct attribute access, reflection, `asdict()`, pickling, or caller-defined serializers are automatically redacted.
