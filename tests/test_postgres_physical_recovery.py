...
def test_pitr_immediate_is_consistent_state_not_end_of_archive() -> None:
    """pitr+immediate is a recovery_target=immediate stop, not replay-to-end-of-WAL."""
    profile = _profile(
        backup_method="pitr",
        recovery_target_kind="immediate",
        wal_archive_required=True,
...
def test_parse_rejects_surrogate_text() -> None:
    """Lone surrogates must stay inside the package JSON error, not UnicodeEncodeError."""
    with pytest.raises(
        PostgresPhysicalRecoveryError,
        match="^invalid PostgreSQL physical recovery profile JSON$",
    ):
