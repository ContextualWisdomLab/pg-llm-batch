...
"""Regression contracts for logical-backup copier descriptor ownership."""
...
def _open_output(tmp_path: Path) -> tuple[Path, int]:
    ...
    path = tmp_path / "pump-fd-ownership.dump"
...
def test_join_signal_does_not_close_descriptor_reused_after_worker_exit(
    ...
    try:
        with pytest.raises(Cancelled):
            logical_backup._run_pg_dump(
                service_name="logical_backup_source",
                output_descriptor=output_descriptor,
                cleanup_descriptor=output_descriptor,
                pg_dump_descriptor=0,
                timeout_seconds=30,
