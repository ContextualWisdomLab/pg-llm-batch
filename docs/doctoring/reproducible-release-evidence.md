...
requires `uv_build==0.12.11`. The exact uv frontend can use its compatible bundled
...
3. Confirm the workflow uses `uv` 0.12.3 and `uv_build==0.12.11` from the exact
   reviewed source.
...
backend requirement remains exactly `uv_build==0.12.11`. Do not treat
