...
"""Regression contract for wheel RECORD size authority."""
...
def _wheel_with_size_defect(defect: str) -> bytes:
    """Return a bounded wheel with one selected RECORD size defect."""
    ...
    if defect == "mismatch":
        package_size = str(len(package_payload) + 1)
    elif defect == "non-decimal":
        package_size = "0x10"
    else:
    ...
    rows = [
        ...
        f"{PACKAGE_MEMBER},sha256={_record_digest(package_payload)},{package_size}",
        f"{record_name},,",
    ]
    record = ("\n".join(rows) + "\n").encode("utf-8")
...
@pytest.mark.parametrize("defect", ("mismatch", "non-decimal"))
def test_release_verifier_binds_present_record_size_to_member_bytes(
    tmp_path: Path,
    defect: str,
) -> None:
    """Reject a present RECORD size unless it is decimal and exact."""
    sdist_bytes = _matching_sdist()
    wheel_bytes = _wheel_with_size_defect(defect)
