...
The counter records completed operations. The histogram unit is seconds and records a nonnegative monotonic duration. Success omits `error.type`; failures use only `checkpoint_conflict`, `validation_error`, or `internal_error`. Package operation spans are storage-agnostic: they do not emit `db.system.name` or claim to be OpenTelemetry database-client spans. When a host needs PostgreSQL client telemetry, the host or database instrumentation layer owns the corresponding database semantic-convention attributes and spans.
