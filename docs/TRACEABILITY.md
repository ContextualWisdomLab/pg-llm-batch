...
| Endpoint-qualified tokenizer metadata | PLANNED #108 | Issue #108; tokenizer metadata currently risks model-only ambiguity across endpoints | implementation must bind tokenizer selection to validated endpoint/model identity, fail closed on missing/conflicting metadata, preserve only unambiguous compatibility, and prove cross-endpoint isolation with realistic PostgreSQL tests | PRD, TRD, data-integrity follow-up |
